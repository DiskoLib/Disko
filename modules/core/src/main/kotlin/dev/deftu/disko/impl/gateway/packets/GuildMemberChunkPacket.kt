...
import dev.deftu.disko.api.utils.maybeGetInteger
...
public class GuildMemberChunkPacket(private val instance: Disko) : ReceivablePacket {

    override fun handlePayloadReceived(gateway: DiskoGateway, data: JsonElement?) {
        if (data == null || !data.isJsonObject) return

        val obj = data.asJsonObject
        val id = obj.maybeGetSnowflake("guild_id") ?: return
        val chunkIndex = obj.maybeGetInteger("chunk_index") ?: return
        val chunkCount = obj.maybeGetInteger("chunk_count") ?: return
        val isLastChunk = chunkIndex == chunkCount - 1
        val members = obj.maybeGetJsonArray("members") ?: return
        instance.guildManager.handleGuildChunk(gateway, id, isLastChunk, members)
    }
